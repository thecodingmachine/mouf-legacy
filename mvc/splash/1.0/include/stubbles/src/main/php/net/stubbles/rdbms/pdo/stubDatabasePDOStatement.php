 ...
 */
stubClassLoader::load('net::stubbles::rdbms::stubDatabaseStatement',
    ...
    public function fetchAll($fetchMode = null, array $driverOptions = array())
    {
        try {
            ...
            if (PDO::FETCH_COLUMN == $fetchMode) {
                return $this->pdoStatement->fetchAll($fetchMode, ((isset($driverOptions['columnIndex']) == false) ? (0) : ($driverOptions['columnIndex'])), null);
            }
            
            return $this->pdoStatement->fetchAll($fetchMode, null, null);
        } catch (PDOException $pdoe) {
            throw new stubDatabaseException($pdoe->getMessage(), $pdoe);
        }
    }
