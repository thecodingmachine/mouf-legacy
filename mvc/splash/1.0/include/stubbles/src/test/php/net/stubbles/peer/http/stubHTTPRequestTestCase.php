...
class stubHTTPRequestTestCase extends PHPUnit_Framework_TestCase
{
    ...
    public function initializeGetRequest()
    {
        ...
        $this->httpRequest->expects($this->once())
                             ...
                             ->with($this->equalTo($mockSocket, stubHTTPRequest::METHOD_GET, stubHTTPRequest::VERSION_1_1))
                             ->will($this->returnValue($mockSocket));
        $response = $this->httpRequest->get(stubHTTPRequest::VERSION_1_1);
        $this->assertType('stubHTTPResponse', $response);
    ...
    public function initializeHeadRequest()
    {
        ...
        $this->httpRequest->expects($this->once())
                             ->method('processHeader')
                             ->with($this->equalTo($mockSocket, stubHTTPRequest::METHOD_HEAD, stubHTTPRequest::VERSION_1_1))
                             ->will($this->returnValue($mockSocket));
    ...
    public function initializePostRequestWithoutBody()
    {
        ...
        $this->httpRequest->expects($this->once())
                             ->method('processHeader')
                             ->with($this->equalTo($mockSocket, stubHTTPRequest::METHOD_POST, stubHTTPRequest::VERSION_1_1))
                             ->will($this->returnValue($mockSocket));
    ...
    /**
     * socket should be created with correct host and port
     *
     * @test
     */
    ...
    public function processHeaderGet()
    {
        ...
        $mockSocket->expects($this->at(6))
                   ->method('write')
                   ->with($this->equalTo('Date: ' . gmdate('D, d M Y H:i:s') . ' GMT' . stubHTTPConnection::END_OF_LINE));
