 ...
 * @version     $Id: stubDateTestCase.php 1869 2008-09-27 14:11:03Z mikey $
 */
...
class stubDateTestCase extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertTrue(date_format(date_create($expected), 'U') === date_format($date->getHandle(), 'U'));
    }
    ...
    /**
     * dates before the year 1582 are 11 days off
     *
     * Quoting Wikipedia:
    ...
    {
        $this->markTestSkipped('PHP does not support this, if we want to support this we have to do it internally.');
        $this->assertDateEquals('1499-12-21T00:00:00+00:00', new stubDate('01.01.1500 00:00 GMT'));
    }

    /**
     * dates before the year 1752 are 11 days off
     *
     * Quoting Wikipedia:
    ...
    public function testTimestamp()
    {
        ...
        $this->assertEquals($d2, new stubDate($d2->format('Y-m-d H:i:sO')));
    }
    ...
    {
        $xmlStreamWriter = stubXMLStreamWriterFactory::createAsAvailable();
        $xmlSerializer   = new stubXMLSerializer();
