 ...
 */
function x_userinput_to_plainstring($str) {
...
function x_plainstring_to_dbprotected($str)
{
	return addcslashes($str, "'\\");
}

function x_plainstring_to_htmloutput($str)
{
	return nl2br($str);
}

function x_plainstring_to_urlprotected($str)
{
	return urlencode($str);
}

function x_userinput_to_htmlprotected($str)
{
	return x_plainstring_to_htmlprotected(x_userinput_to_plainstring($str));
}

function x_dbdate_to_displaydate($strDate, $ccode="en", $long=false)
{
 ...
 */
function x_get($var, $type="string", $compulsory=false, $default_value=false, $origin="R") {
	for ($i=0; $i<strlen($origin); $i++)
	{
		if ($origin{$i} == 'R' || $origin{$i} == 'r')
		{
			...
			if (isset($_REQUEST[$var]) && is_array($_REQUEST[$var])){
				foreach($_REQUEST[$var] as $key=>$value){
					$tab[$key] = x_userinput_to_plainstring($value);
				}
				return $tab;
			}
			...
			{
				if ($type=='string')
					return x_userinput_to_plainstring($_REQUEST[$var]);
				elseif ($type=='int')
				{
					if (!x_check_integer($_REQUEST[$var]))
					{
		...
		elseif ($origin{$i} == 'S' || $origin{$i} == 's')
		{
			if (isset($_SESSION[$var]))
			{
				...
				elseif ($type=='int')
				{
					if (!x_check_integer($_SESSION[$var]))
					{
						echo '<b>Error!</b> The '.$var.' field must be an integer';
						exit;
					}
		...
		elseif ($origin{$i} == 'C' || $origin{$i} == 'c')
		{
			if (isset($_COOKIE[$var]))
			{
				...
				elseif ($type=='int')
				{
					if (!x_check_integer($_COOKIE[$var]))
					{
						echo '<b>Error!</b> The '.$var.' field must be an integer';
						exit;
					}
 ...
 * @return unknown
 */
function x_string_to_sql($var) {
	if ($var !== null)
			return "'".x_plainstring_to_dbprotected($var)."'";
		else 
			return 'NULL';
}
