...
/**
 * The TDBM_LessOrEqualFilter class translates into an "<=" SQL statement (or a "IS NULL" statement if the value to compare is null).
 * 
 * @Component
...
class TDBM_LessOrEqualFilter implements TDBM_FilterInterface {
	 ...
	 * @param string $value
	 */
	public function TDBM_LessOrEqualFilter($tableName=null, $columnName=null, $value=null) {
		$this->tableName = $tableName;
		$this->columnName = $columnName;
		$this->value = $value;
	}
	...
	public function toSql(DB_ConnectionInterface $dbConnection) {
		...
		if ($this->value === null) {
			throw new TDBM_Exception("Error in TDBM_LessOrEqualFilter: trying to compare $this->tableName.$this->columnName with NULL.");
		}
