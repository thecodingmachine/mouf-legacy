...
/*$dsn = array(
 ...
 $db = new TDBM_Connection($dsn, $options);*/

class TDBM_Connection {
	/**
	 * The main database.
	 *
	 * @var TDBM_Connection
	...
	private function findRootSequenceTable($table_name){
		...
		throw new TDBM_Exception('Cannot find Root Sequence Table for other DBMS than MySQL 5+ and PostGreSQL 7+');
	...
	public function getParentTable($table_name){
		...
		throw new TDBM_Exception('Unable to retrieve Parent Table for database type '.$this->dsn['phptype'].'<br />\nCurrently, only MySQL 5+ and PostGreSQL 7+ are supported.');
	...
	private function getPrimaryKey($table_name) {
		...
		throw new TDBM_Exception('Unable to retrieve Primary Key for database type '.$this->dsn['phptype'].'<br />\nCurrently, only MySQL 5+ and PostGreSQL 7+ are supported.');
	}
	...
	function getConstraintsOnTable($table_name,$column_name=false) {
		...
		throw new TDBM_Exception('Unable to retrieve Constraints for database type '.$this->dsn['phptype'].'<br />\nCurrently, only MySQL 5+ and PostGreSQL 7+ are supported.');
	}
...
class TDBM_Object {
	...
	static public function deleteObject(TDBM_Object $object) {
		...
		if ($object->DBM_Object_state != "new")
		{
			...
			$result = TDBM_Connection::$main_db->query($sql);

			if (TDBM_Connection::$main_db->affectedRows() != 1)
			throw new TDBM_Exception("Error while deleting object from table ".$object->db_table_name.": ".TDBM_Connection::$main_db->affectedRows()." have been affected.");
