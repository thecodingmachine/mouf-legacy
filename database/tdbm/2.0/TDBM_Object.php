...
class TDBM_Object implements ArrayAccess, Iterator {
	 ...
	 * TDBM_Service class that will create TDBM_Objects for you.
	 * 
	 ...
	 * @param mixed $id
	 */
	public function __construct(TDBM_Service $tdbmService, $table_name, $id=false) {
	...
	public function _dbLoadIfNotLoaded() {
		if ($this->TDBM_Object_state == "not loaded")
		{
			...
			{
				throw new TDBM_Exception("Could not retrieve object from table \"$this->db_table_name\" with ID \"".$this->TDBM_Object_id."\".");
			}
	...
	public function save() {
		...
		if ($this->TDBM_Object_state == "new") {
			...
			if (count($pk_array)==1 && !$pk_set) {
				...
				$this->TDBM_Object_id = $this->db_row[$pk_array[0]];
			}
			...
			$this->TDBM_Object_id = $this->db_connection->getInsertId($this->db_table_name,$primary_key);
			$this->db_row[$primary_key] = $this->TDBM_Object_id;
			}*/
		...
		} else if ($this->TDBM_Object_state == "loaded" && $this->db_modified_state==true) {
			//$primary_key = $this->getPrimaryKey();
			// Let's first get the primary keys
			$pk_table = $this->getPrimaryKey();
			// Now for the object_id
	...
	public function discardChanges() {
		...
		if ($this->TDBM_Object_state == "deleted")
	 ...
	 * This method returns the status of the TDBM_Object.
