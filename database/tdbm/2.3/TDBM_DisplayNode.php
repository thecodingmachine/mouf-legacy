...
class TDBM_DisplayNode {
	...
	public function draw($x, $y, $left_px, $top_px) {
		...
		$mybox_width_px = $this->width*TDBM_DisplayNode::$box_width + ($this->width-1)*TDBM_DisplayNode::$interspace_width;
		$my_x_px = $left_px + TDBM_DisplayNode::$left_start + $x*(TDBM_DisplayNode::$box_width + TDBM_DisplayNode::$interspace_width);
		$my_y_px = $top_px + TDBM_DisplayNode::$top_start + $y*(TDBM_DisplayNode::$box_height + TDBM_DisplayNode::$interspace_height);
		...
		*/
		$str .= "<div style='position:absolute; left:".$my_x_px."px; top:".$my_y_px."px; width:".$mybox_width_px."px; height:".TDBM_DisplayNode::$box_height."; background-color:gray; color: white; text-align:center; border:".TDBM_DisplayNode::$border."px solid black'>\n<b>".$this->table_name."</b></div>";

		if ($this->keyParent != null) {
			$my_x_px_line = $my_x_px + TDBM_DisplayNode::$box_width/2;
			$my_y_px_line = $my_y_px - TDBM_DisplayNode::$interspace_height;
			$str .= "<div style='position:absolute; left:".$my_x_px_line."px; top:".($my_y_px_line+TDBM_DisplayNode::$border)."px; width:2px; height:".(TDBM_DisplayNode::$interspace_height-TDBM_DisplayNode::$border)."; background-color:black; '></div>\n";
			...
			$str .= "<div style='position:absolute; left:".($my_x_px_line+2)."px; top:".($my_y_px_line+TDBM_DisplayNode::$border*2)."px; background-color:#EEEEEE; font-size: 10px'>$top_key</div>\n";
			$str .= "<div style='position:absolute; left:".($my_x_px_line+2)."px; top:".($my_y_px_line+TDBM_DisplayNode::$interspace_height-TDBM_DisplayNode::$text_height)."px; background-color:#EEEEEE; font-size: 10px'>$bottom_key</div>\n";
		}
